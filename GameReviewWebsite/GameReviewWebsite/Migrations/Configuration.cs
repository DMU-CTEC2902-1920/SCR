namespace GameReviewWebsite.Migrations
{
    using System;
    using System.Data.Entity;
    using System.Data.Entity.Migrations;
    using System.Linq;
    using GameReviewWebsite.Models;
    using System.Collections.Generic;

    internal sealed class Configuration : DbMigrationsConfiguration<GameReviewWebsite.Models.AnGameContext>
    {
        public Configuration()
        {
            AutomaticMigrationsEnabled = false;
            ContextKey = "GameReviewWebsite.Models.AnGameContext";
        }

        protected override void Seed(GameReviewWebsite.Models.AnGameContext context)
        {
            //developer
            var developers = new List<AnDeveloper>
            {
                new AnDeveloper {DeveloperId=1, Name = "George", Description="A goood developer"},
                new AnDeveloper {DeveloperId=2, Name = "Steve", Description="A Okay developer"},
                new AnDeveloper {DeveloperId=3, Name = "Micheal", Description="A decent developer"},
                new AnDeveloper {DeveloperId=4, Name = "John", Description="A bad developer"}

            };

            developers.ForEach(developer => context.
                                  AnDevelopers.
                                  AddOrUpdate(a => a.DeveloperId, developer));


            context.SaveChanges();


            //User
            var users = new List<AnUser>
            {
                new AnUser {UserId =1, Email="cjs@hotmail.com", Password="Test1!"},
                new AnUser {UserId =2, Email="pjs@hotmail.com", Password="Test2!"},
                new AnUser {UserId =3, Email="ljs@hotmail.com", Password="Test3!"},
                new AnUser {UserId =4, Email="mjs@hotmail.com", Password="Test4!"}
            };

            users.ForEach(user => context.
                                  AnUsers.
                                  AddOrUpdate(b => b.UserId, user));
            context.SaveChanges();
            //genres
            var genres = new List<AnGenre>
            {
                new AnGenre {GenreID=1, Name = "Adventure", Description="Lots of adventure"},
                new AnGenre {GenreID=2, Name = "Action", Description="Lots of action"},
                new AnGenre {GenreID=3, Name = "MMO", Description="Lots of multiplayer"},
                new AnGenre {GenreID=4, Name = "Rpg", Description="Lots of roleplaying"}
            };

            genres.ForEach(genre => context.
                                  AnGenres.
                                  AddOrUpdate(c => c.GenreID, genre));
            context.SaveChanges();

            



            //comment
            var comments = new List<AnComment>
            {
                new AnComment {CommentId =1, CommentText="Nice guy", UserId=9},
                new AnComment {CommentId =2, CommentText="Bad guy", UserId=10}
            };

            comments.ForEach(comment => context.
                                  AnComments.
                                  AddOrUpdate(d => d.CommentId, comment));
            context.SaveChanges();

            //commentreplys
            var commentreplies = new List<AnCommentReply>
            {
                new AnCommentReply {CommentReplyId =1, CommentReplyText="I Agree", UserId=11, CommentId=1},
                new AnCommentReply {CommentReplyId =2, CommentReplyText="I Dissagree", UserId=12, CommentId=2}
            };

            commentreplies.ForEach(commentreply => context.
                                  AnCommentReplys.
                                  AddOrUpdate(d => d.CommentReplyId, commentreply));
            context.SaveChanges();

            //review
            var reviews = new List<AnReview>
            {
                new AnReview {ReviewId =1, ReviewText="Nice game", UserId=1},
                new AnReview {ReviewId =2, ReviewText="Bad game", UserId=2}
            };

            reviews.ForEach(review => context.
                                  AnReviews.
                                  AddOrUpdate(d => d.ReviewId, review));
            context.SaveChanges();

            //reviewreplys
            var reviewreplies = new List<AnReviewReply>
            {
                new AnReviewReply {ReviewReplyId =1, ReviewReplyText="I Agree", UserId=1, ReviewId=1},
                new AnReviewReply {ReviewReplyId =2, ReviewReplyText="I Dissagree", UserId=2, ReviewId=2}
            };

            reviewreplies.ForEach(reviewreply => context.
                                  AnReviewReplys.
                                  AddOrUpdate(d => d.ReviewReplyId, reviewreply));
            context.SaveChanges();

            var games = new List<AnGame>
            {
                new AnGame{GameId=1, Name="Minecraft", Description="Fun", Platform="Pc", ImgSrc="https://media.playstation.com/is/image/SCEA/minecraft-starter-collection-squareboxart-01-ps4-11dec19-en-us?$native_nt$",
                    Price=2, DeveloperId=1, GenreID=1, ListReview=reviews, ListReviewReply=reviewreplies},
                new AnGame{GameId=2, Name="Rust", Description="Infuriating", Platform="Pc", ImgSrc="data:image/jpeg;base64,",
                    Price=2, DeveloperId=2, GenreID=1, ListReview=reviews,ListReviewReply=reviewreplies },
                new AnGame{GameId=9, Name="Pubg", Description="battleRoyal!", Platform="Pc", ImgSrc="https://www.extremetech.com/wp-content/uploads/2017/12/PUBG-Feature-640x353.jpg",
                    Price=2, DeveloperId=1, GenreID=1, ListReview=reviews, ListReviewReply=reviewreplies},
            };

            games.ForEach(game => context.
                                  AnGames.
                                  AddOrUpdate(e => e.GameId, game));
            context.SaveChanges();

        }
    }
}
